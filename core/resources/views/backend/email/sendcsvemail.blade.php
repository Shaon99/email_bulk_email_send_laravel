                ...
                <div class="col-12 col-md-12 col-lg-12">
                    <div class="card p-3">
                        <div class="form-group">
                            <a href="{{ route('admin.DownloadCSV') }}" class="btn btn-primary float-right">{{
                                __('Download Sample CSV File') }}
                            </a>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.sendBulkEmail') }}" method="POST"
                                enctype="multipart/form-data" class="needs-validation" novalidate="" id="form">
                                @csrf

                                <div class="form-group">
                                    <label for="">{{ __('Email Address (csv file*)') }}</label>
                                    <input type="file" class="form-control" name="csv_file" required="">
                                    <div class="invalid-feedback">
                                        {{ __('email (csv) can not be empty') }}
                                    </div>
                                </div>
